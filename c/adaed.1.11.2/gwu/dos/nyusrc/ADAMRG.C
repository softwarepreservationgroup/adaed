...
printf("post header\n");
    ...
    if (n_error) {
/*  AVL  */
		AVL_fp = fopen("GWADA.SMM","w");
		fprintf(AVL_fp, "%d %d %d %d %d %d total,Lex,syntax,semantic,bind,gen ",  n_error, n_lex_err, 
			n_syntax_err, n_semantic_err, n_binding_err, n_generator_err);
		fclose(AVL_fp);
