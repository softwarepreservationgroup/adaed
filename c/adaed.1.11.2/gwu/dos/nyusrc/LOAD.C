...
long load_slots(char *fname, IFILE **ifile, Axq axq)			/*;load_slots*/
{
	...
	*ifile = AXQFILE;		/* store file pointer */
	...
	code_slots = (char **) ecalloct(code_slots_dim, sizeof(char *),
	  "code_slots");
#ifdef MONITOR
	code_slots_file = (char **) ecalloct(code_slots_dim, sizeof(char *),
	  "code_slots");
	code_slots_package = (char **) ecalloct(code_slots_dim, sizeof(char *),
	  "code_slots");
#endif
...
#ifdef MONITOR
		code_slots_file[slot->slot_number] = slot->slot_file;
		code_slots_package[slot->slot_number] = slot->slot_package;
#endif
	}
