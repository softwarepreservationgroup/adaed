  ...
  /*;instantiate_generics*/
{
	...
	for (i=first_named; i <= gn; i++) {
		...
		if (exists) {
			...
			if(NATURE(g_name) == na_procedure || NATURE(g_name) == na_function){
				name = dcl_get(DECLARED(SCOPE_OF(g_name)), N_VAL(id_node));
				if (set_size(OVERLOADS(name)) > 1)
#ifdef ERRNUM
					errmsgn(62, 63, id_node);
#else
					errmsg(
					  "named associations not allowed for overloaded names",
					  "12.3(3)", id_node);
#endif
			}
...
#ifdef ERRNUM
			id_errmsgn(64, g_name, 58, current_node);
#else
...
#ifdef ERRNUM
		str_errmsgn(66, op_name, 67, node);
#else
...
#ifdef ERRNUM
			l_errmsgn(68, 69, 43, actual_node);
#else
...
#ifdef ERRNUM
			l_errmsgn(72, 73, 74, actual_node);
#else
...
#ifdef ERRNUM
			l_errmsgn(72, 75, 71, actual_node);
#else
			errmsg_l("Instantiation of generic in out parameter ",
...
#ifdef ERRNUM
			l_id_errmsgn(76, 77, g_name, 74, actual_node);
#else
			errmsg_l_id(
...
#ifdef ERRNUM
		id_errmsgn(78, g_name, 58, current_node);
#else
...
#ifdef ERRNUM
		id_errmsgn(79, g_name, 80, current_node);
#else
...
#ifdef ERRNUM
		id_errmsgn(81, g_name, 58, current_node);
#else
...
#ifdef ERRNUM
		id_errmsgn(82, g_name, 58, current_node);
#else
...
#ifdef ERRNUM
		id_errmsgn(83, g_name, 84, current_node);
#else
...
#ifdef ERRNUM
		id_errmsgn(85, g_name, 84, current_node);
#else
...
#ifdef ERRNUM
		id_errmsgn(86, g_name, 84, current_node);
#else
...
#ifdef ERRNUM
			l_id_errmsgn(87, 88, g_name, 84, current_node);
#else
...
#ifdef ERRNUM
			id_id_errmsgn(89, designated_formal, g_name, 90, current_node);
#else
...
#ifdef ERRNUM
			l_errmsgn(91, 92, 90, current_node);
#else
...
#ifdef ERRNUM
		id_errmsgn(93, g_name, 94, current_node);
#else
  ...
  Symbolmap type_map)						/*;valid_array_instance*/
{
	...
	if ( !is_array(actual_type)) {
#ifdef ERRNUM
		id_errmsgn(95, g_name, 96, current_node);
#else
...
#ifdef ERRNUM
		id_errmsgn(97, g_name, 96, current_node);
#else
...
#ifdef ERRNUM
		id_errmsgn(98, g_name, 96, current_node);
#else
...
#ifdef ERRNUM
		id_errmsgn(99, g_name, 96, current_node);
#else
		...
		if (exists) {
#ifdef ERRNUM
			l_id_errmsgn(100, 101, g_name, 96, current_node);
#else
...
#ifdef ERRNUM
			l_errmsgn(102, 92, 96, current_node);
#else
...
#ifdef ERRNUM
			id_errmsgn(103, g_name, 104, instance);
#else
