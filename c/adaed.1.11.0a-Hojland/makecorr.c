...
void make_correction(struct two_pool *STATE_STACK)		/*;make_correction*/
{
    ...
    if ( CANDIDATES[MERGE] != NULL ) {
		 ...
		 */
		 ...
		 tokens[tokind]->ptr.token->loc = ct->ptr.token->loc ;
	...
	/************************** S P E L L   S U B S T *********************/
		...
		tokmod->ptr.token->loc = ct->ptr.token->loc ; 
	...
	/***************************** I N S E R T *******************************/
		...
		/* curtoken->prev->ptr.token->loc = pt->ptr->loc ; */
		...
		tokens[tokind]->ptr.token->loc = pt ;
		tokmod = NULL;		
			...
			if (( tmp_symbol == SEMI_SYM  )
			  ...
			  || ( tmp_symbol > EOFT_SYM ) ) {
				...
				*/
				...
				/*
				syntax_err(&PREVTOK->ptr.token->loc,
				  &PREVTOK->ptr.token->loc,msg);
				*/
				...
				syntax_err(&PREVTOK->ptr.token->loc,
				  &PREVTOK->ptr.token->loc,msg);
				syntax_err(&curtok->ptr.token->loc,&curtok->ptr.token->loc,msg);
				*/
			}
		}
		else {
